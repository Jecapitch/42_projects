...
/*   Updated: 2024/12/25 18:07:37 by jpiscice         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	return (1 * (cmp_first(a) == '>') + 2 * (cmp_first(b) == '<'));
}
...
int	cmp_last_2_first(t_list_circ *stack, char cmp_sign)
{
	...
	return (1 * (cmp_sign == 'a') * (valstart > val1 && valstart > val2) \
			+ 2 * (cmp_sign == 'b') * (valstart < val1 && valstart < val2) \
			+ 4 * (cmp_sign == 'a') * (valstart < val1 && valstart > val2) \
			+ 8 * (cmp_sign == 'b') * (valstart > val1 && valstart < val2));
}

int	cmp_2_last_combine(t_list_circ *a, t_list_circ *b)
{
	return (cmp_last_2_first(a, 'a') + cmp_last_2_first(b, 'b'));
}
