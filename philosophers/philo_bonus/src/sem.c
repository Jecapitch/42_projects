...
/*   Updated: 2025/06/20 07:29:58 by jpiscice         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	open_semaphores(t_data *data)
{
	...
	data->dead_sem = sem_open(DEAD_SEM, O_CREAT | O_EXCL, 0644, 1);
...
int	close_semaphores(t_data *data)
{
	...
	if (data->dead_sem && data->dead_sem != SEM_FAILED)
		ret |= sem_close(data->dead_sem);
	...
	return (data->start_sem = NULL, data->end_sem = NULL, \
			data->dead_sem = NULL, data->done_sem = NULL, \
			data->fork_sem = NULL, data->glob_sem = NULL, \
			data->msg_sem = NULL, \
			ret);
}
