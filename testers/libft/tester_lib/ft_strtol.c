...
/*   Updated: 2026/04/13 12:38:18 by jepiscic         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	tester_ft_atoi(const char *s)
{
	if (!s)
	{
	...
	return (tester_ft_strtol(s, BASE10));
}
...
static int	check_base(const char *base, int base_hash[256])
{
	...
	{
		c = (t_uchar)base[b];
		if (base_hash[c] != -1 || c == '+' || c == '-' || tester_ft_isspace(c))
			return (0);
		base_hash[c] = b;
		b++;
	}
...
t_ulong	tester_ft_strtoul(const char *s, const char *base)
{
	...
	return (tester_ft_strtol(s, base));
}
