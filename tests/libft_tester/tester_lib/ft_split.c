...
char	**tester_ft_split(const char *s, char c)
{
	...
	sep[0] = c;
	sep[1] = '\0';
	return (tester_ft_split_set(s, sep));
}
...
char	**tester_ft_split_set(const char *s, const char *set)
{
	...
	split_str = tester_ft_calloc((tester_ft_count_words_set(s, set) + 1), sizeof(char *));
	if (!split_str)
		return (NULL);
	i = 0;
	while (*s)
	{
		...
		if (!(*s))
			break ;
		split_str[i] = tester_ft_substr(s, 0, tester_ft_strlen_delim_set(s, set));
		if (!split_str[i++])
		{
			tester_ft_free_str_tab(split_str);
			return (NULL);
		}
